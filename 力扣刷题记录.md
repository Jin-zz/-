...
class Solution {
    public String minNumber(int[] nums) {
        ...
        //自定义排序规则
        //str1.compareTo(str2) 比较两个字符串大小
        //Arrays.sort(strs, (x, y) -> (x + y).compareTo(y + x)); //谁在前面的时候小 该数就小 x+y<y+x -> x<y
...
动态规划：前`i-1`个字符有`f(i-1)`种翻译，则前`i`个字符串至少也有`f(i-1)`种翻译，当`i`与`i-1`组成的字符串对应的数字属于`[10,25]`，则再加上`f(i-2)`种翻译

- `f[i] = f[i-1] + if(10<=substr(i-1,i)<=25) f[i-2];`
- `f[i-2] = f[i-1] , f[i-1] = f[i];`
- 初始状态 `f[-1] = 0; f[0] = 1; f[1] = 1; f[2] = 1 or 2`  //这里i为前i个字符，即str[i-1];

```java
class Solution {
    public int translateNum(int num) {
        //f[i] = f[i-1] + if(<10=substring(i-1,i)<=25) f[i-2];
        int[] F = new int[3];
        F[0] = 0; //a
        F[1] = 1; //b
        F[2] = 1;  //c 未知
        String str = String.valueOf(num);
        if(str.length()<2) return F[str.length()];
        //从第二个数开始
        for(int i=1;i<str.length();i++){
            ...
            String temp = str.substring(i-1,i+1);//左闭右开
            if(temp.compareTo("10")>-1 && temp.compareTo("25")<1){
                F[2] = F[0]+F[1]; //c = 
            }
            else F[2] = F[1];     //c =      
...
1. DFS + 回溯 慢 超时
   ...
   ```java
       //速度很慢的DFS+回溯
   void getroad(int temp,int i,int j,int[][] grid){
           
       temp += grid[i][j];
       if(i==grid.length-1&&j==grid[0].length-1){
           if(temp>res) res = temp; //res全局变量
       }else{
           if(i+1<grid.length) getroad(temp,i+1,j,grid);
           if(j+1<grid[0].length) getroad(temp,i,j+1,grid);
       }
   }
   ```

2. 最基础的 动态规划：原矩阵m*n，dp矩阵(mx1) * (nx1)

   `dp[i][j] = Math.max(dp[i-1][j],dp[i][j-1]) + grid[i-1][j-1]`

   优化：int[] dp=int[n+1]，则`dp[j]` = `dp[i-1][j]`的值，`dp[j-1]` = `dp[i][j-1]`的值
...
**解答:**动态规划/双指针+Set/Hashmap

> Hashmap直接存储（字符，字符最后一次出现的位置index），则长度等于 = i - index（简化）
...
class Solution {
    public int lengthOfLongestSubstring(String s) {
        ...
        int res = 0;
        int start = 0;
        Set<Character> set = new HashSet<>(); //set维护一个无重复字符的表
        while(i<dp.length){
            //dp 以i结尾的最长子串长度
            while(set.contains(chs[i-1])){
                //set包含
                set.remove(chs[start]);
                start++;
            }
            set.add(chs[i-1]);
            //dp[i] = i-start; //有重复
...
- 丑数应当是另一个丑数乘以2、3、5的结果，如果我们有一个无限大的丑数的集合nums，它应该是由<u>每一个</u>丑数，再乘以2、3、5得到的
- 设置三个指针，分别表示已经乘过2、3、5的数，设为d2、d3、d5
- 则对于i来说，`nums[i] = min(nums[d2]*2, nums[d3]*3, nums[d5]*5); d2/3/5++;` 直到i=n-1
- 初始化，d2 = 0, d3 = 0, d5 = 0; 最开始丑数集合中只有一个丑数 nums[0]=1，比较`[nums[0]*2, nums[0]*3, nums[0]*4]`，d2满足条件，表明nums[d2]这个数已经乘过2了，d2=d2+1=1，nums[1] = 2；接下来比较 `[nums[1]*2, nums[0]*3, nums[0]*4]`……
...
class Solution {
    ...
    public int nthUglyNumber(int n) {
        ...
        int d2 = 0, d3 = 0, d5 = 0;
        for(int i=1;i<n;i++){
            int tmp = Math.min(Math.min(nums[d2]*2,nums[d3]*3),nums[d5]*5);
            if(tmp==nums[d2]*2) d2++;
            if(tmp==nums[d3]*3) d3++;
            if(tmp==nums[d5]*5) d5++;
            nums[i] = tmp;
...
class Solution {
    public boolean isNumber(String s) {
        ...
        System.out.println(s.substring(beStart,beEnd));
        System.out.println(s.substring(afStart,afEnd));
        char[] be = s.substring(beStart,beEnd).toCharArray();
        char[] af = s.substring(afStart,afEnd).toCharArray();
        
        System.out.println("before:"+beforeE(be));
        System.out.println("after:"+afterE(af));
        if(beforeE(be)&&afterE(af)) return true;

        return false;
    }

    boolean beforeE(char[] str){
        if(str.length==0){
        ...
        int douNum = 0;
        int start=0;
        if(isSign(str[start])) start++;
        for(int i=start;i<str.length;i++){
            if(isNum(str[i])) continue;
            if(isDou(str[i])){
                douNum++;
                if(douNum>1||str.length==1) return false;
                continue;
            }       
            return false;
        }
...
class Solution {
    public int search(int[] nums, int target) {
        ...
        //tar=nums[mid]时 指向右指针
        //如果nums中存在tar 最终start=last=最小的tar
        //如果nums中不存在tar:
        //如果nums[0]>tar,start=last=nums[0]
        //如果nums[nums.length-1]<tar,start=last=nums[nums.length-1]
        //else : start=last=大于tar的最小的数
        while(start<last){
            int mid = (start+last)/2;
            if(nums[mid]>=target) last = mid; //相等时取右边界 最终找到最左元素
            else start = mid+1;
        }
        int count=0;
        while(last>=0&&last<nums.length&&nums[last]==target){
            count++;
            last++;
        }
        return count;
...
class Solution {
    public String reverseLeftWords(String s, int n) {
        ...
        reverseStr(arr,0,n-1);  //反转前半部分
        reverseStr(arr,n,arr.length-1);  //反转后半部分
        reverseStr(arr,0,arr.length-1);  //全部反转
        return new String(arr);
    }

    void reverseStr(char[] arr, int i,int j){
        while(i<j){
            char tmp = arr[i];
            arr[i] = arr[j];
            arr[j] = tmp;
...
class Solution {
    public double[] twoSum(int n) {
        ...
        int[][] dp = new int[n+1][6*n+1]; //dp
        //初始化
        for(int i = 1;i <= 6;i++){
            dp[1][i] = 1;
        }
        //dp[i][j] : 共i个骰子 投出j点数 = dp[i-1][j-1:j-6] 之和
        //当j=9时：
        //第i个骰子投出1 = 共i-1个骰子 投出j-1=8
        //第i个骰子投出2 = 共i-1个骰子 投出j-2=7
        //第i个骰子投出3 = 共i-1个骰子 投出j-3=6
        //第i个骰子投出4 = 共i-1个骰子 投出j-4=5
        //第i个骰子投出5 = 共i-1个骰子 投出j-5=4
        //第i个骰子投出6 = 共i-1个骰子 投出j-6=4
        //则 共i个骰子 投出j=9点数  = 上面之和
...
class Solution {
    ...
    int isAtoB(int a, int b, int zero){
        if(a==b) return -1;
        a++;
        while(a<b){
            a++;
...
class Solution {
    public int lastRemaining(int n, int m) {
        //f(n,m) = (f(n-1,m)+m)%n 表示n个人 每次杀死第m个人 最后那个人的下标
        //f(1,m) = 0
...
class Solution {
    public int maxProfit(int[] prices) {
        ...
        //dp[i] = max(price[i]-min_price[i],dp[i-1])
        int[] dp = new int[prices.length];  //可以直接用res替代
        dp[0] = 0;
        int min = prices[0];
...
![image-20200908154452327](https://gitee.com/sparkle_zz/markdown-pics/raw/master/image-20200908154452327.png)
 ...
 */
class Solution {
    Stack<TreeNode> pAncestor = new Stack<>(); //存储路径
    Stack<TreeNode> qAncestor = new Stack<>();
    public TreeNode lowestCommonAncestor(TreeNode root, TreeNode p, TreeNode q) {
        boolean pres = getAncestors(root,p,pAncestor);
        boolean qres = getAncestors(root,q,qAncestor);

        TreeNode res = new TreeNode(0);
        //比较路径
   		while(!pAncestor.empty()&&!qAncestor.empty()&&pAncestor.peek().val==qAncestor.peek().val){
            res = pAncestor.pop();
            qAncestor.pop();
        }
        return res;
    ...
    //先序遍历得到路径 最后进栈为根节点
    boolean getAncestors(TreeNode root, TreeNode x,Stack Ancestor){
        ...
        if(root.val==x.val){
            ...
            if(getAncestors(root.left,x,Ancestor)){
                Ancestor.add(root);
                return true;
            };
            if(getAncestors(root.right,x,Ancestor)){
                Ancestor.add(root);
                return true;
            };
        }
...
- List 里面只存储头节点，只将所有情况的头节点存入即可
- 根节点为1，2，…，n
- 当 i 为根节点时，构造左子树（start,i-1）,右子树（i+1,last）
- 继续递归左子树（start,i-1），右子树（i+1,last）,返回存储左右子树头节点的List
- 则 i 为根节点时，构造出的不同的二叉搜索树的数量为 size(leftList) * size(rightList)
...
class Solution {
    ...
    List<TreeNode> getTreeNode(int start, int last){  
        List<TreeNode> res =  new LinkedList<TreeNode>();

        //这里不能等于 
        //相等时getTreeNode(1,1)指左子树只有一个节点(1)
        //getTreeNode(n,n)指右子树只有一个节点(n)
        if(last<start){ 
            res.add(null);
            return res;
        }
        ...
        for(int i = start;i <= last;i++){
            ...
            List<TreeNode> righttree = getTreeNode(i+1,last);
...
**解答：**动态规划：找到对于长度为n的序列，以i为根节点的二叉搜索树的个数

```java
class Solution {
    public int numTrees(int n) {
        //二叉搜索树 左节点<根节点<右节点
        //F(i,n) 表示 长度为n的序列,以i为根节点的二叉搜索树的个数
        //G(n) 表示 长度为n的序列 共有多少个二叉搜索树
        //则有 G(n) = 总和 F(i,n)(i=1~n)
        //动态递归：F(i,n) = G(i-1)*G(n-i) 
        //即F(3,7) = G(2)*G(4)
        //即F(3,7)可以分为[1-2],[4-7],因为只考虑个数 所以以序列[4-7]构成的二叉搜索树的个数==[1-4]
        //则 G(n) = 总和 G(i-1)*G(n-i) (i=1~n)
...
class Solution {
    public int[] intersect(int[] nums1, int[] nums2) {
        ...
        //         if(val==1) map.remove(nums2[j]);
        //         else map.put(nums2[j],--val);
        //     }
        // }
        ...
        while(!A.isEmpty()&&!B.isEmpty()){
            if(A.peek()<B.peek()) A.poll();
            else if(A.peek()>B.peek()) B.poll();
            else {
                res.push(A.poll());
                B.poll();
            }
